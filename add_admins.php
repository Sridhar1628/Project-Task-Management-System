...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO admins (admin_id, name, email, mobile, password) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $admin_id, $name, $email, $mobile, $password);

    if ($stmt->execute()) {
        echo "<script>alert('Admin added successfully!');window.location.href='manage_admins.php';</script>";
    } else {
        echo "<script>alert('Error Admin Adding!');window.location.href='manage_admins.php';</script>";
    }
...
<form action="" method="POST">
    ...
    <input type="password" name="password" pattern="^(?=.*[A-Z])(?=.*[@$!%*?&])(?=.*[0-9]).{8,}$" title="Password must contain at least one uppercase letter, one special character, one number, and be at least 8 characters long" required>
    
    <button type="submit">Add Admin</button><br><br>
    <button type="reset" style="background: linear-gradient(to right, #001f3f, #003366, #004080);color:white;">Reset</button><br><br>
    <button type="button" style="background-color:red;color:white;" onclick="window.location.href='manage_admins.php'">Cancel</button>
</form>
</div>
</body>
</html>